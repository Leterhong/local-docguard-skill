...
> **默认纯本地（赛事合规）**：`model_config.yaml` 的 `security.local_only` 默认为 `true`，此时云端被锁死、文档 100% 本地闭环；如需「混合智能」云端增强，将其设为 `false` 并开启 `providers.cloud.enabled` 即可，云端不可用时自动降级本地规则引擎。
